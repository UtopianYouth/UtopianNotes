...
给出入栈和出栈顺序，学会用辅助栈来模拟，判断出栈顺序是否正确，题意和上一题一样。
...
> 理解字符串的匹配规则即可，匹配一个、匹配零个、匹配多个都需要分情况列出来。
...
由于对节点进行根节点查找的时候，存在较多的递归调用，可以采用路径压缩算法，使得查的过程时间复杂度优化到：O(1)；除了路径压缩算法，也可以将节点高度较小的集合，合并到节点高度较大的集合中。
...
/*
    并查集模板
    - 1. 查询（判断两个节点是否属于一个集合中）
    - 2. 合并（将不同集合的节点合并到一个集合中）
    - 3. 初始化
...
int findSet(int num) {
    // 普通版本, 不断递归查找到根节点处
    return num == father[num] ? num : findSet(father[num]);
    // 路径压缩版本
    //return num == father[num] ? num : father[num] = findSet(father[num]);
}
...
> **算法应用部分**
>
> - 并查集：判断两个节点是否连通，适用于有向图和无向图，两个集合的合并和查找都需要会；
> - 最小生成树：带权无向图算法，找到最小的n-1条边，最小权值，连通n个节点；
>   - prim：基于`min_dist[n]`数组实现，是基于节点的，时间复杂度为O(n^2)；
>   - kruskal：**基于并查集实现**，每次选择权值最小的边加入到并查集中，直到所有的节点都在同一个集合里面；
> - 拓扑排序算法：有向图算法，基于节点的入度，不断将入度为0的节点找出来排序即可；
> - 单源最短路径算法：Dijkstra算法，和prim算法一样，都是基于节点的算法，核心是维护一个`min_dist[N]`数组，朴素版实现的时间复杂度是O(n^2)，基于邻接矩阵；优化思路是使用邻接表存储图，优先队列查找最近未访问的节点，时间复杂度降为O(n*logn)。
...
### [LC 1](https://leetcode.cn/problems/two-sum/description/)：两数之和

### [LC 15](https://leetcode.cn/problems/3sum/)：三数之和
