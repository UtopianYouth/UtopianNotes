...
>
> 扩展之后，每一个comet服务器都会有房间订阅管理的逻辑，但是有些comet服务器上是没有ws连接的，这就会导致消息广播给指定房间或者广播到所有用户时，job层默认推送到所有的comet服务器，无法实现精准推送（好像是一个死结啊，非常奇怪）。
>
> 
>
> **负载均衡: Nginx/LVS做4层或7层负载均衡**
>
> 主要是均衡一下comet服务器的ws连接，job层和logic层，由于是后端整体服务，其实每一层的节点都是有限个的，比如10个，这些微小的连接不用做负载均衡。
>
> 
>
...
>
> 需要精准推送才需要，比如私聊，或者指定群聊（这样的话，job层多了与redis交互的业务逻辑，本质还是处理了业务，emm有点奇怪。
>
> 
>
> **Kafka分区扩展: 增加分区数，提升消息吞吐量**
>
> 这个还不太熟悉，需要了解 Kafka 的工作原理
...
> **懒汉式单例模式: C++11保证static局部变量初始化线程安全**
>
> 项目中用到的单例模式还挺多的，单例模式的优势是，在多线程的系统中，保证了在线程安全的前提下，系统运行过程中，对应类的对象资源实例只有一个。
