...
| **性能足够** | 对于大多数应用，epoll的性能已经足够，所以异步IO其实用的不多 |
| **编程简单** | 同步I/O，调试容易 |
...
| **性能** | O(n) | O(n) | O(1) |
| **fd数据结构** | bitmap (fd_set) | 数组 (pollfd) | 红黑树+就绪链表实现 |
| **事件通知** | 全量拷贝 | 全量拷贝 | 增量通知 |
| **跨平台** | ✅ | ✅ | ❌ (Linux独有) |
...
注：每个socket都有自己的等待队列，等待队列存放了注册的监听事件（不止epoll），及其事件发生后执行的回调函数入口地址。
...
​	该方法用的最多，用户程序通过用户缓冲区缓存不完整的数据，当判断完整的数据符合协议栈的格式的时候，就认为获得了完整的数据，以webserver解析HTTP请求为例，分布式实时消息推送系统解析ws帧也是同理。
...
其实webserver项目的架构有点像nginx，属于单reactor + 线程池处理，线程池的任务队列中，存储的任务对象主要的逻辑是解析HTTP请求，生成HTTP响应和资源文件。
